...
/**
 * Adapter de persistencia: implementa el puerto del dominio delegando en
 * Panache (repository pattern) y mapeando entre Product (POJO) y
 * ProductEntity (JPA). Los tipos Panache/JPA no salen de aquí.
 */
@ApplicationScoped
public class ProductRepositoryAdapter implements ProductRepository {
    ...
    public Product save(Product product) {
        ProductEntity entity =
                new ProductEntity(product.name(), product.sku(), product.quantity());
    ...
    public List<Product> findAll() {
        return products.listAll(Sort.by("id"))
                .stream()
                .map(this::toDomain)
                .toList();
    }
